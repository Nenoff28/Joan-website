...
export default function Product() {
  ...
  return <Layout>
    ...
    <main className="product-page">
      ...
      {isZoomed && <div className="image-zoom-modal" role="dialog" aria-modal="true" aria-label="Приближен изглед на продукта" onClick={() => setIsZoomed(false)}><button type="button" className="image-zoom-close" onClick={() => setIsZoomed(false)} aria-label="Затвори приближения изглед"><X size={20} /></button><img src={activeImage} alt={product.imageAlt || product.name} onClick={(event) => event.stopPropagation()} /></div>}
    </main>
  </Layout>;
}
