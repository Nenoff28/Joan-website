...
import { ArrowRight, ChevronLeft, ChevronRight, FileText, MapPin, Maximize2, Minimize2, Pause, Play, PlayCircle, Search, ShieldCheck } from "lucide-react";
...
export default function Home() {
  ...
  function handleBrochureTouchEnd(event: React.TouchEvent<HTMLDivElement>) { const touch = event.changedTouches[0]; const start = touchStartRef.current; touchStartRef.current = null; if (!start) return; const horizontalDistance = touch.clientX - start.x; const verticalDistance = touch.clientY - start.y; if (Math.abs(horizontalDistance) < 48 || Math.abs(horizontalDistance) < Math.abs(verticalDistance)) return; setBrochurePaused(true); setBrochurePage(activeBrochurePage + (horizontalDistance < 0 ? 1 : -1)); }
  function handleBrochureKeyDown(event: React.KeyboardEvent<HTMLDivElement>) { if (event.key === "ArrowLeft") { event.preventDefault(); setBrochurePaused(true); setBrochurePage(activeBrochurePage - 1); } if (event.key === "ArrowRight") { event.preventDefault(); setBrochurePaused(true); setBrochurePage(activeBrochurePage + 1); } if (event.key === "Escape" && brochureFullscreen) closeBrochureFullscreen(); }
