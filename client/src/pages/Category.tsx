...
function CataloguePage({ showAll = false }: { showAll?: boolean }) {
  ...
  const pageSize = 48;
  const cataloguePage = useCataloguePage({ page, pageSize, categorySlug: showAll ? undefined : category?.slug, path: selectedPath.length ? selectedPath : undefined, query: query || undefined, brand: selectedBrand === "Всички марки" ? undefined : selectedBrand, availability: [inStock ? "in_stock" : null, enquiry ? "on_request" : null].filter((value): value is "in_stock" | "on_request" => Boolean(value)), minPrice: minPrice ? Number(minPrice) : undefined, maxPrice: maxPrice ? Number(maxPrice) : undefined, sort: sort as "relevance" | "price-asc" | "price-desc" | "name-asc" | "name-desc" });
