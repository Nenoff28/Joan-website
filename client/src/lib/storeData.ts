...
function createTestProduct(category: (typeof categories)[number], seed: ProductSeed, index: number, imageIndex: number): Product {
  ...
  return {
    ...
    features: ["Каталожен артикул", "Наличност по потвърждение", "Информация от Жоан"],
    description: `${name} е каталожен артикул на Жоан. Цената и наличността се потвърждават от екипа на магазина.`,
  };
}
