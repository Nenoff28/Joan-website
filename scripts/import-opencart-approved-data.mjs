...
async function main() {
  ...
  try {
    ...
    for (const row of products) {
      ...
      const specialPrice = cell(row.price_special) ? decimal(row.price_special) : null;
      const primaryPriceBgn = specialPrice ?? normalPrice;
      const priceEur = (Number(primaryPriceBgn) / 1.95583).toFixed(2);
      const oldPriceBgn = specialPrice ? normalPrice : null;
      const oldPriceEur = oldPriceBgn ? (Number(oldPriceBgn) / 1.95583).toFixed(2) : null;
