...
import { legacyProductSlug, preferredProductSlug, uniqueProductSlug } from "./lib/product-slug.mjs";
...
async function main() {
  ...
  try {
    ...
    const existingProductRows = await queryRows(connection, "SELECT legacyProductId, slug, legacyPublicSlug FROM catalogue_products WHERE legacyProductId IS NOT NULL");
    const existingProductByLegacyId = new Map(existingProductRows.map((row) => [Number(row.legacyProductId), row]));
    const reservedProductSlugs = new Set(existingProductRows.filter((row) => !String(row.slug).startsWith("legacy-")).map((row) => row.slug));
