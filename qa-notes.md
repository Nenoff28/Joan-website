...
Facebook asset and FAQ-navigation QA — 2026-08-20: the supplied blue Facebook asset is served from deployment-safe storage and replaces the generated footer icon without any visible adjacent text. The visible desktop `FAQ` control now appears immediately after “Контакти”; browser activation from the homepage opened `/returns#returns-faq` and landed at the FAQ section (`1703px` above the viewport), after the initial validation identified and corrected the missing anchor identifier.

FAQ route and supplied-icon follow-up — 2026-08-20: mobile browser automation at a 375px viewport opened the drawer, found the visible `FAQ` link, activated it, and reached `/returns#returns-faq` with the FAQ anchor at the viewport top (`scrollY: 2736px`). The same check confirmed that the supplied footer image loaded from `/manus-storage/joan-facebook-icon_f29d2620.webp`. Full-page 375px screenshots of the home and return pages confirm the text-free blue icon appears at the footer and all new return-page content stays within the viewport. TypeScript passed, all 37 Vitest tests passed, and the production build completed successfully. The build retains its existing advisory warning for a large generated JavaScript chunk.
