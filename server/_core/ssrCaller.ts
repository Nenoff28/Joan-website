...
export async function buildSsrPrefetch(req: Request, res: Response): Promise<SsrPrefetch> {
  ...
  return { metadata: () => caller.catalogue.metadata(), page: (input) => caller.catalogue.page(input), product: (input) => caller.catalogue.product(input), brochure: () => caller.catalogue.brochure() };
}
